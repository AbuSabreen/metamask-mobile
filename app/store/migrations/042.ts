...
export default function migrate(state: unknown) {
  if (!isObject(state)) {
    captureException(
      new Error(`Migration 42: Invalid state error: '${typeof state}'`),
    );
    return state;
  }
  ...
  if (!isObject(state.engine)) {
    captureException(
      new Error(
        `Migration 42: Invalid engine state error: '${typeof state.engine}'`,
      ),
    );
    return state;
  }
  ...
  if (!isObject(state.engine.backgroundState)) {
    captureException(
      new Error(
        `Migration 42: Invalid engine backgroundState error: '${typeof state
  ...
  if (!isObject(networkControllerState)) {
    captureException(
      new Error(
        `Migration 42: Invalid NetworkController state error: '${typeof networkControllerState}'`,
      ),
    );
    return state;
  }
  ...
  if (!networkControllerState.providerConfig.chainId) {
    captureException(
      new Error(
        `Migration 42: NetworkController providerConfig chainId not found: '${JSON.stringify(
